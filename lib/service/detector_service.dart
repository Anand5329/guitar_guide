...
class _DetectorServer {
  ...
  static List<Map<String, dynamic>> postProcess(
    ...
    for (var i = 0; i < rawOutput.first.length; i++) {
      ...
      // Step 3: Convert bounding box to pixel values
      double left = (x - w / 2) * imageWidth;
      double top = (y - h / 2) * imageHeight;
      double right = (x + w / 2) * imageWidth;
      double bottom = (y + h / 2) * imageHeight;

      // Store the box as a map for easier access later
      boxes.add({
        'class': classId,
        'confidence': confidence * classConfidence,
        'rect': [left, top, right, bottom], // bounding box coordinates
      });
    }

    // Step 4: Apply Non-Maximum Suppression (NMS)
    return applyNMS(boxes, nmsThreshold);
  }
  ...
  static List<Map<String, dynamic>> applyNMS(
    ...
    List<Map<String, dynamic>> nmsBoxes = [];
    List<bool> isBoxRemoved = List.generate(boxes.length, (_) => false);

    for (int i = 0; i < boxes.length; i++) {
      if (isBoxRemoved[i]) continue; // Skip if the box is already removed
      Map<String, dynamic> currentBox = boxes[i];
      nmsBoxes.add(currentBox);

      for (int j = i + 1; j < boxes.length; j++) {
        if (isBoxRemoved[j]) continue;

        Map<String, dynamic> compareBox = boxes[j];

        // Calculate the intersection over union (IoU)
        double iou = calculateIoU(currentBox['rect'], compareBox['rect']);

        if (iou > threshold) {
          // Mark the box as removed if the IoU is above the threshold
          isBoxRemoved[j] = true;
        }
      }
    }
